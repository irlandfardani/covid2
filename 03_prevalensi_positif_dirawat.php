...
function style(f) {
	...
	data = dataCovid[KODE];
	var prdirawat = Number(data.pr_positif)-(Number(data.pr_positif_sembuh)+Number(data.pr_positif_md));
	// console.log(data);
	return {
		weight: 1,
		opacity: 1,
  ...
  function onEachFeature(f, layer) {
		...
		var popUp='<table>'+
					...
					'<tr>'+
						...
						'<td>Prevalensi Pasien Dirawat</td>'+
						'<td>'+prdirawat+'</td>'+
					'</tr>'+
					'</table>';
  	...
  	$.ajax({
  		...
  		success:function(data){
  			...
  			for (i=0;i<features.length;i++) {
  				...
  				//var prdirawat=Number(attributes.positif)+Number(attributes.md);
				var prdirawat = Number(attributes.pr_positif)-(Number(attributes.pr_positif_sembuh)+Number(attributes.pr_positif_md));
				...
				var list='<h5>'+attributes.kabupaten+'</h5>'+
							...
							'<span class="text-warning">Prevalensi Pasien Positif Dirawat: '+prdirawat+'</span>'+
