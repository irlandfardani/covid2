...
function style(f) {
	...
	data = dataCovid[KODE];
	var kasusaktif = Number(data.positif)-(Number(data.positif_sembuh)+Number(data.positif_md));
	// console.log(data);
	return {
		weight: 1,
		opacity: 1,
  	...
  	$.ajax({
  		...
  		success:function(data){
  			...
  			for (i=0;i<features.length;i++) {
  				...
  				//var kasusaktif=Number(attributes.positif)+Number(attributes.md);
				var kasusaktif = Number(attributes.positif)-(Number(attributes.positif_sembuh)+Number(attributes.positif_md));
				...
				var list='<h5>'+attributes.kabupaten+'</h5>'+
							...
							'<span class="text-warning">Kasus Aktif: '+kasusaktif+'</span>-'+
