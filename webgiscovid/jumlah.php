...
<body>
	<header>
		...
		<h3>WebGIS Penyebaran Covid-19 di Indonesia</h3>
	</header>
...
<script type="text/javascript">
  ...
  var dataCovid=[];
  var geojson=[];
  map.attributionControl.addAttribution('Created by &copy; <a href="https://t.me/didinstudio">Didin Studio</a>');
  getData();
...
function style(f) {
	...
	data = dataCovid[KODE];
	var positifmd = Number(data.positif)-(Number(data.positif_sembuh)+Number(data.positif_md));
	// console.log(data);
	return {
		weight: 1,
		opacity: 1,
...
// update info
  ...
  function onEachFeature(f, layer) {
		...
		data = dataCovid[KODE];
		var positifmd=Number(data.positif)+Number(data.md);
		var popUp='<table>'+
					'<tr>'+
  	...
  	$.ajax({
  		...
  		success:function(data){
  			...
  			for (i=0;i<features.length;i++) {
  				...
  				var positifmd=Number(attributes.positif)+Number(attributes.md);
				...
				var list='<h5>'+attributes.kabupaten+'</h5>'+
							...
							'<span class="text-warning">Positif + Meninggal : '+positifmd+'</span>';
